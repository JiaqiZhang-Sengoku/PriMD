...
def train_or_eval_model(args, model, reg_loss, cls_loss, dataloader, optimizer=None, train=False, first_stage=True, mark='train'):
    ...
    for data_idx, data in enumerate(dataloader):
        ...
        ## add cuda for tensor
        if cuda:
            audio_host, text_host, visual_host = audio_host.to(device), text_host.to(device), visual_host.to(device)
            audio_guest, text_guest, visual_guest = audio_guest.to(device), text_guest.to(device), visual_guest.to(device)
        ...
        full_input_features = generate_inputs(audio_host, text_host, visual_host, \
        ...
        '''
        ## forward
        hidden, out, out_a, out_t, out_v, weight_save, aux_losses = model(
            masked_input_features[0],
            input_features_mask[0],
            umask,
            first_stage,
            teacher_inputfeats=full_input_features[0],
        )
        ...
        if dataset in ['IEMOCAPFour', 'IEMOCAPSix']:
            loss_main = cls_loss(lp_, labels_, umask)
            loss_a = cls_loss(lp_a, labels_, umask)
            loss_t = cls_loss(lp_t, labels_, umask)
            loss_v = cls_loss(lp_v, labels_, umask)
            if first_stage:
                loss = loss_main + (loss_a + loss_t + loss_v) / 3.0 + aux_losses['teacher_loss']
            else:
                loss = loss_main + aux_losses['student_loss']
        if dataset in ['CMUMOSI', 'CMUMOSEI']:
            loss_main = reg_loss(lp_, labels_, umask)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--primitive-capacity', dest='primitive_capacity', type=int, default=64, help='PriMD primitive memory capacity')
    parser.add_argument('--retrieval-dim', dest='retrieval_dim', type=int, default=64, help='PriMD retrieval projection dimension')
    parser.add_argument('--k-max', dest='k_max', type=int, default=16, help='PriMD maximum retrieved primitives')
    parser.add_argument('--gumbel-tau', dest='gumbel_tau', type=float, default=0.5, help='PriMD dynamic retrieval temperature')
    parser.add_argument('--kd-temp', dest='kd_temp', type=float, default=4.0, help='PriMD distillation temperature')
    parser.add_argument('--lambda-align', dest='lambda_align', type=float, default=0.1, help='SSSD shared-alignment loss weight')
    parser.add_argument('--lambda-indep', dest='lambda_indep', type=float, default=0.01, help='SSSD independence loss weight')
    parser.add_argument('--lambda-vq', dest='lambda_vq', type=float, default=0.1, help='DPMC vector-quantization loss weight')
    parser.add_argument('--vq-beta', dest='vq_beta', type=float, default=0.2, help='DPMC commitment weight')
    ...
    for ii in range(args.num_folder):
        ...
        for epoch in range(args.epochs):
            ...
            if epoch == args.stage_epoch-1:
                model.start_student_stage()
                end_first_stage_time = time.time()
                print("------- Teacher and primitive memories are frozen; starting the PriMD student stage! -------")
